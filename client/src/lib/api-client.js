...
apiClient.interceptors.request.use(
  (config) => {
    ...
    } else if (process.env.NODE_ENV !== 'production' && config.method !== 'get') {
      // Log warning for POST/PUT/DELETE requests without token
...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    // Handle cached request interception (legacy - kept for backward compatibility)
    ...
    if (!error.response && (error.code === 'ECONNABORTED' || error.message === 'Network Error' || error.code === 'ECONNREFUSED')) {
      // No cached fallback - let the app handle the error
